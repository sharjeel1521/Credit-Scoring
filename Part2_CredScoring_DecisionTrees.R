...
n = nrow(dd)
learn = sample(1:n, size=round(0.63 * n))
nlearn = length(learn)
ntest = n - nlearn
...
ct1 = rpart(Y ~ ., data = dd[learn,], method="class",
...
# the optimal tree corresponds to a cp=0.0018
ct1$cptable[min.xe,]

# Now that we know that the 'best' tree has cp=0.0018
...
library(gmodels)
CrossTable(credit_test$default, ct2.learn,
           prop.chisq = FALSE, prop.c = FALSE, prop.r = FALSE,
           dnn = c('actual default', 'predicted default'))

# create a vector with predicted status
ct2.learnp = rep("", nlearn)
ct2.learnp[ct2.learn < 0.5] = "pred_neg"
ct2.learnp[ct2.learn >= 0.5] = "pred_pos"
# let's make a table
Y_learn = table(dd$Y[learn], ct2.learnp)
Y_learn

# classification error
100 * sum(diag(Y_learn)) / nlearn


# calculate error rate in the testing sample
# (this will give a matrix)
ct2.test = predict(ct2, newdata=dd[-learn,])
# create a vector with predicted status
ct2.testp = rep("", ntest)
ct2.testp[ct2.test < 0.5] = "pred_neg"
ct2.testp[ct2.test >= 0.5] = "pred_pos"
# let's make a table
Y_test = table(dd$Y[-learn], ct2.testp)
# classification error
100 * sum(diag(Y_test)) / ntest
...
ct3.learnp[ct3.learn < 0.5] = "pred_neg"
ct3.learnp[ct3.learn >= 0.5] = "pred_pos"
# let's make a table
table(dd$Status[learn], ct3.learnp)
# classification error
100 * sum(diag(table(dd$Y[learn], ct3.learnp))) / nlearn
...
# create a vector with predicted status
ct3.testp = rep("", ntest)
ct3.testp[ct3.test > 0.5] = "pred_neg"
ct3.testp[ct3.test <= 0.5] = "pred_pos"
# let's make a table
table(dd$Y[-learn], ct3.testp)
# classification error
100 * sum(diag(table(dd$Y[-learn], ct3.testp))) / ntest
...
tapply(Y.test == "good", pred.test, sum)
ac_true.pos = 100 * cumsum(rev(as.numeric(table))) / npos
