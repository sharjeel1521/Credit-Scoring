...
library(VIM)
library(mice)
s<-md.pattern(dd(13:24))
plot(s)
m_plot <- aggr(dd[c(13:24)], col=c('navyblue','yellow'),
                 numbers=TRUE, sortVars=TRUE,
                 labels=names(dd[c(13:24)]), cex.axis=.7,
...
dd[is.na(dd)] <- 0
# ================================================================================
# scaling the variable contineous variables
# ================================================================================
# This step has to be done after the knn imputation, otherwise you'll get
# scatter values 


# normalize the numerical variables in wbcd data using MinMax
PAY_LIMIT<-dd[2]
dd1<-dd[2]

summary (dd[2])
boxplot(PAY_LIMIT,xlab="PAY_limit", ylab="Percentage")
dd1<-dd[13:24] 
dd1[13]<-dd[2]
dd1 <- as.data.frame(apply(dd1, MARGIN = 2, FUN = function(x)
...
boxplot (dd1, main = "MinMax")

# ================================================================================
# Lambda scaling the variable contineous variables
# ================================================================================
# This step will ensure that lambda scaling is beter then min-max1111111111111111222222222222222222222222222222222222222222222222222222222222222220i ;p11
# scatter values "
...
boxplot (dd2, xlab="Bill_Amounts", ylab="Percentage")


#normalize the numberical variables in credit data using lambda
library(DMwR)


d2 <- as.data.frame(apply(d2, MARGIN = 2, FUN = function(x)
...
# Inspect using boxplots
boxplot (d2, main = "Soft Max, lambda = 7")

scor<-cor(d2,use="complete.obs",method="spearman")
library(corrplot)
M <- cor(scor)
